...
/** Reuse the direct-role boundary without coupling the tool to union positions. */
const workflowRoleAction = z
	.object({
...
export const createWorkflowRoleTool = (
	...
	const schema = workflowRoleAction.omit({ action: true }).extend({
		role: z
			.enum([first.name, ...rest.map(({ name }) => name)])
			.describe(
				workers
...
Delegate implementation to phase workers or a directly selected worker through these tools; do not execute implementation in the router. The engine owns task state, role execution, checkpoints, and reports. Worker tools still authorize real effects; workflow controls do not bypass permissions or project constraints.
