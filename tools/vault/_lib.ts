...
// always goes through `acceptVaultPath` so the traversal guard runs
// before any I/O. `walkVault` is the in-package recursive directory
// walker; we deliberately avoid pulling in fast-glob/picomatch and
// stick to `node:fs/promises` to keep the dep surface flat.
...
import { error, ok } from "../common/result.ts";
import type { VaultAccessor, VaultPathError } from "../common/vault-root.ts";
// oxlint-disable-next-line no-duplicate-imports
import { acceptVaultPath, resolveAccessorRoot } from "../common/vault-root.ts";
...
export const acceptRoot = async (
	...
	acceptVaultPath(accessor, relPath);
...
// v1 is body+frontmatter assembly only; the per-kind template-merge
// described in the design's vault-write doc-arm waits for the concrete
// VaultResolver (which owns the templates dir layout). Callers that
// need template defaults can read+merge themselves and pass the merged
// frontmatter in.
